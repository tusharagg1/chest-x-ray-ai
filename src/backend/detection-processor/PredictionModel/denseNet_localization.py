...
def getimgs(imgdir):
    test_X = []
    # get dicom images from directory
    imgfiles = getfilenames(imgdir, '.dcm')
    for fname in imgfiles:
        image_path = os.path.join(imgdir, fname)
        img = getimgdata(image_path)
...
def getbbvalues(imgdir):
    test_X = getimgs(imgdir)
    model = DenseNet121(8)#.cuda()
    model = torch.nn.DataParallel(model)
    model.load_state_dict(torch.load("PredictionModel/DenseNet121_aug4_pretrain_WeightBelow1_1_0.829766922537.pkl", map_location=torch.device('cpu')))
    ...
    for index in range(len(test_dataset)):
        input_img = Variable((test_dataset[index]).unsqueeze(0), requires_grad=True) # deleted .cuda() after unsqueeze(0)
    ...
    prediction_dict = {}
    file_list = getfilenames(imgdir, '.dcm')
    ...
    with open("bounding_box.txt","w") as f:
        for i in range(len(prediction_dict)):
            ...
            print(os.path.join(imgdir, fname), len(prediction))
            f.write('%s %d\n' % (os.path.join(imgdir, fname), len(prediction)))
