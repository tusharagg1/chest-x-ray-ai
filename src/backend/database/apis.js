...
import { getDatabase, ref as refd, set, onValue, update } from "firebase/database";
import { getStorage, ref as refs, uploadBytes, listAll } from "firebase/storage"
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, onAuthStateChanged, signOut, updateProfile, reload } from "firebase/auth";
 ...
 */
export const createNewUser = async (userName, email, password, firstName, lastName, medInsts) => {
  try {
    ...
    writeUserData(userCredentials.user.uid, userName, email, firstName, lastName, medInsts);
  }
  catch (error) {
    console.log(error);
  }
 ...
 * @param {[string]} medInsts  User's associated medical institution(s).
 */
export async function writeUserData(userId, userName, email, firstName, lastName, medInsts) {
  ...
  .then(() => {
    console.log("User data for %s was written to the database successfully!", userName);
  })
  .catch((error) => {
    console.log("User data for %s was not written to the database successfully. The following error was returned:\n%s", userName, error);
  });
}
...
export function getUserData(userId, onlyOnce = false) {
  ...
  onValue(getUserRef, (snapshot) => {
    ...
    console.log("UserId: %s\nUserName: %s\nEmail: %s\nFirst Name: %s\nLast Name: %s\nMedical Institution(s): %s",
      userId, userData.userName, userData.email, userData.firstName, userData.lastName, userData.medInsts);
  }, {
    onlyOnce: onlyOnce
  });
...
export function writePatientData(patientId, firstName, lastName, email, patientDiseaseRisks = {}) {
  ...
  .then(() => {
    console.log("Patient data for %s was written to the database successfully!", patientId);
  })
  .catch((error) => {
    console.log("Patient data for %s was not written to the database successfully. The following error was returned:\n%s", patientId, error);
  });
}
...
export function writeScanResultsData(scanResultsId, patientId, firstName, lastName, email, patientDiseaseRisks, chestXrayImage) {
  ...
  .then(() => {
    console.log("Scan results data for %s for %s was written to the database successfully!", scanResultsId, userId);
  })
  .catch((error) => {
    console.log("Scan results data for %s for %s was not written to the database successfully. The following error was returned:\n%s", scanResultsId, userId, error);
  });
