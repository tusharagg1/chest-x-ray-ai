...
test('BT01. Create Staff User', async () => {
  await createANewUser("StaffUser99", "staff.user99@mailsac.com", "atom!@#",
    "Staff", "User99", [], false
  )
  .then((user_id) => {
...
test('BT04. Create Patient Record', async () => {
  await createANewPatient(99, 99, "Patient", "No99", "03-30-2001", "M",
...
test('BT16. Create Staff User with Invalid Email', async () => {
  await createANewUser("StaffUser99", "staff.user99mailsac.com", "atom!@#",
    "Staff", "User99", [], false
  )
  .then((user_id) => {
